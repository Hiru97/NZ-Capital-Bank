...
try {
    $stmt = $conn->prepare("SELECT * FROM transactions 
                          WHERE account_id = ? AND type = 'bill_payment'
                          ORDER BY created_at DESC");
    $stmt->execute([$sender_account['id']]);
    $bill_payments = $stmt->fetchAll();
} catch (PDOException $e) {
    $bill_payments = []; // If there's an error, set bill payments to an empty array
}
    ...
    if (empty($bill_type)) {
        ...
        try {
            ...
            $stmt->execute([
                $sender_account['id'],
                $amount,
                "Bill payment for {$bill_type}",
                $remark
            ]);

            $conn->commit();
            $success = "Bill payment successful! NZ$ " . number_format($amount, 2) . " paid for {$bill_type}.";
        } catch (PDOException $e) {
            $conn->rollBack();
            $error = "Bill payment failed due to a system error. Please try again later.";
        } catch (Exception $e) {
            $conn->rollBack();
            $error = $e->getMessage();
        }
...
<head>
    ...
    <title>NZ Capital Bank - Pay Bill</title>
    ...
    <div class="dashboard-grid">
        ...
        <div class="card">
            ...
            <form action="pay_bill.php" method="POST">
                ...
                <button type="submit" class="transfer-button">Pay Bill</button>
            </form>
        </div>
